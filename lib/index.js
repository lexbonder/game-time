...
window.addEventListener('keydown', e => {
  ...
  if (e.key === 'w') {
    lilyArray.forEach( lily => lily.color = 'lightgreen')
  }
})
...
function fillLeftLane() {
  for (var i = 0; i < 15; i++) {
    if (i < 5) {
      laneLeft.push(
        new Vehicle(20 + i * 85, 465, 25, 20, .6, 'BlueViolet'));
    } else if (i < 10) {
      laneLeft.push(
        new Vehicle(20 + (i - 5) * 85, 396, 25, 20, .6));
    } else if (i < 15) {
      laneLeft.push(
        new Vehicle(20 + (i - 10) * 85, 330, 25, 20, 1, 'blue'));
    }
  }
}
...
function fillWaterLeft() {
  for (var i = 0; i < 8; i++) {
    if (i < 3) {
      logsLeft.push(
        new Log(20 + i * 95, 260, 25, 23, 1, 'Magenta'));
    } else if (i < 5) {
      logsLeft.push(
        new Log(200 + (i - 3) * 200, 236, 100, 20, .6, 'OrangeRed'));
    } else if (i < 8) {
      logsLeft.push(
        new Log(20 + (i - 5) * 195, 160, 50, 20, 1, 'DarkOrange'));
    }
  }
}
...
function reset() {
  ...
  lilyArray.forEach( lily => lily.color = 'transparent')
